...
This repository contains a simple electric car Lookup API. The app is largely irrelevant as it just used to illustrate the stages to test and deploy code via a GitHub Actions pipeline.

This workflows expects users to merge into main and have the pipeline test and deploy from the initial branch and PR creation through to the production deploy.  This will use a Trunk based workflow rather that GitFlow.
